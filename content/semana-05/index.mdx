...
¿No lograste completar la semana o quieres comparar tu código con la solución? Descarga el estado del proyecto al terminar la Semana 5:

**[⬇️ Descargar entregable Semana 5 (.zip)](https://github.com/abecerraguz/blog-cms/archive/refs/tags/semana-05.zip)**

**Qué incluye:** el proyecto completo en estructura monorepo — `frontend/` con Zustand store, hooks, context, componentes y tipos; `backend/` con servidor Express, CORS y rutas `/api/v1/articulos` devolviendo mock data; `package.json` raíz con `concurrently`.
...
npm run dev              # arranca frontend (5173) + backend (3001) en paralelo
```
