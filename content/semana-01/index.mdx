...
frontend/src/
└── types/
    └── index.ts     ← NUEVO: interfaces Articulo, Categoria, Tag, EstadoArticulo
```

> **Dónde va:** `src/types/index.ts` es el único archivo de tipos del frontend. Todas las interfaces del dominio van aquí y se importan desde cualquier parte del proyecto con `import type { Articulo } from '@/types'`. Nunca definas tipos dentro de un componente o página.
...
¿No lograste completar la semana o quieres comparar tu código con la solución? Descarga el estado del proyecto al terminar la Semana 1:

**[⬇️ Descargar entregable Semana 1 (.zip)](https://github.com/abecerraguz/blog-cms/archive/refs/tags/semana-01.zip)**

**Qué incluye:** la configuración inicial de Vite + TypeScript, el archivo `src/types/index.ts` con las interfaces `Articulo`, `Categoria`, `Tag` y `EstadoArticulo`, y `src/data/mockData.ts` con datos de prueba.

**Setup después de descomprimir:**
```bash
cd blog-cms
npm install
npm run dev   # solo el frontend por ahora
```
