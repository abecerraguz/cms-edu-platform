...
import type { CategoriaSemana } from "@/types";
...
export default function NuevaSemanaPage() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!res.ok) {
      ...
      setError((json as { error?: string }).error ?? "Error al crear la semana");
      setSubmitting(false);
      return;
    }
  ...
  return (
    <div className="max-w-2xl">
      <div className="flex items-center gap-3 mb-8">
        ...
        <h1 className="text-xl font-bold text-zinc-900 dark:text-white">Nueva semana</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 p-6 space-y-5">
        ...
        <div>
          ...
          <input
            ...
            placeholder="Ej: Fundamentos de TypeScript"
          />
        </div>
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="Breve descripción del contenido de la semana"
          />
        </div>
        ...
        <div className="flex gap-6">
          <label className="flex items-center gap-2 text-sm text-zinc-700 dark:text-zinc-300 cursor-pointer">
            <input type="checkbox" name="disponible" defaultChecked className="rounded" />
            Publicada
          </label>
          <label className="flex items-center gap-2 text-sm text-zinc-700 dark:text-zinc-300 cursor-pointer">
            <input type="checkbox" name="esExamen" className="rounded" />
        ...
        <div className="flex items-center justify-end gap-3 pt-2 border-t border-zinc-100 dark:border-zinc-800">
          ...
          >
            {submitting ? "Guardando…" : "Crear semana"}
          </button>
        </div>
      </form>
    </div>
